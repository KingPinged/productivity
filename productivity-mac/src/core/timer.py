...
class PomodoroTimer:
    ...
    def __init__(
        ...
        Args:
            ...
            afk_check: Callback that returns True if user is AFK. When AFK during
                       work sessions, timer countdown is silently paused.
        """
    ...
    def resume(self) -> None:
        """Resume the timer from pause, restoring the correct state (work or break)."""
        with self._lock:
            if self._state == TimerState.PAUSED:
                self._running = True
                self._set_state(self._paused_from_state)
                self._timer_thread = threading.Thread(target=self._run_timer, daemon=True)
                self._timer_thread.start()

    @property
    def paused_from_state(self) -> str:
        """Get the state we were in before pausing."""
        return self._paused_from_state
    ...
    def _run_timer(self) -> None:
        ...
        while self._running:
            ...
            with self._lock:
                ...
                # Check for AFK during work sessions
                # If AFK, silently skip countdown (don't decrement time)
                # This is seamless - user won't be notified, timer just pauses
                if self._state == TimerState.WORKING and self.afk_check:
                    try:
                        if self.afk_check():
                            # User is AFK - skip this tick but keep checking
                            # Still call on_tick so UI stays responsive
                            if self.on_tick:
                                self.on_tick(self._time_remaining)
                            continue
                    except Exception:
                        pass  # If AFK check fails, continue normally
