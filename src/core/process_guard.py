"""
Process guard - monitors and respawns the app if killed.
Runs as a separate hidden process to make the app harder to terminate.
"""
...
def set_process_priority_low():
    """Set this process to low priority to stay hidden."""
    try:
        p = psutil.Process()
...
class ProcessGuard:
    ...
    Runs with a deceptive process name.
    """
...
def find_main_app() -> Path:
    ...
    # When compiled, the guard exe is in the same folder as main app exe
    if getattr(sys, 'frozen', False):
        # Running as compiled exe
        exe_dir = Path(sys.executable).parent
        # Look for RuntimeBroker.exe (or other disguised name)
        for name in ["RuntimeBroker.exe", "ProductivityTimer.exe", "main.exe"]:
            candidate = exe_dir / name
            if candidate.exists():
                return candidate
        # Fall back to any exe that's not us
        for exe in exe_dir.glob("*.exe"):
            if exe.name != Path(sys.executable).name:
                return exe
    else:
        # Running as Python script
